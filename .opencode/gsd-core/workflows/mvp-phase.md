...
Normalize per `@.opencode/gsd-core/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
Run three sequential `question` calls. Each is free-text. After all three, assemble into the canonical sentence per `@.opencode/gsd-core/references/user-story-template.md`:
...
Run the SPIDR rules from `@.opencode/gsd-core/references/spidr-splitting.md`. Briefly:
