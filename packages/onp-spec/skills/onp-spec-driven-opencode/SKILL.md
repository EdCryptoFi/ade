...
description: Native spec-anchored development for opencode — the specification stays true because it is mechanically audited against the code. Use when planning features, implementing with verification, or auditing an implementation against the spec. Triggers "specify feature", "new feature", "implement", "audit spec", "verify", "execution plan", "run in parallel", "what has no test", "lessons learned". Flow Specify → Design → Tasks → Plan → Execute → Audit → Learn, with traceability story→acceptance criterion→task→test, executable definition of done (each acceptance criterion becomes an annotated test), assumptions and questions as first-class citizens, verifiable constitution (LGPD/education preset), lessons learned with mechanical backing and an execution plan with OPTIONAL PARALLELISM: the agent presents the recommended plan and ALWAYS asks WHICH tasks the user wants to parallelize (lanes with git worktrees + headless `opencode run` sessions via --paralelizar, or one task after another via --sequencial) and ALWAYS confirms the MODELS and EFFORTS per task before executing — the tokens and the license are the user's; they lock the cost with --modelo/--esforco on the plan or per task via `onp-spec tarefa`. Warns that the execution runs in background and, during it, posts the progress table in the chat every 1 minute (what is running and what is not) + overall summary — with a full summary at the end. Integration with opencode's native resources (skills directory, TodoWrite, interactive questions, explicit invocation via skill name). Mechanical engine EMBEDDED in the skill (zero install — runs with the environment's node).
license: MIT
metadata:
...
# onp-spec-driven — the specification that stays true (opencode)
...
## Interaction — use the opencode harness for the user

This skill runs natively inside opencode (it is loaded from the project's
`.opencode/skills/onp-spec-driven/`). Use the native resources to keep the
flow visible and interactive, without turning into bureaucracy:
...
- **Explain what you did and where you are**: after EVERY action, say in simple
  ...
  file?" nor "what now?".
- **Todo list (TodoWrite)**: when starting a feature, create one todo per phase
  ("Specify homework-submission", "Write tests", "Implement", "Audit"...). On
  large features, one todo per task (T-xxx) in the Execute phase. Keep the
...
- **Interactive questions (AskUserQuestion)**: when an open question or an
  ...
  concrete options (include your recommendation) — and record the answer in the
  spec (status `answered`/`confirmed`). Don't accumulate questions silently or
...
resolved **relative to this SKILL.md's directory** (opencode reports the skill's
...
`plano <feature> [--agents opencode] [--paralelizar T-xxx,T-yyy] [--sequencial]` ·
...
- In `tasks.md`, break into **tasks (T-xxx)**. Each task has `Refs:` (the
  ...
  allowed). Optional per-task fields: `Model:` (an opencode model,
  `provider/model`, e.g.: `anthropic/claude-sonnet-5`) and `Effort:`
  (low|medium|high|max — the plan converts it to opencode's `--variant`). Model
  and effort are **your proposals** — whoever has the final say is the user, in
  the cost confirmation of the Plan phase; to adjust without editing the file:
...
- **MODEL and EFFORT are the USER's choice — confirm BEFORE executing.**
  ...
  `onp-spec plano <feature> --modelo openai/gpt-5.6-luna --esforco baixo` (the
  two lock ALL tasks and win over tasks.md and config); **(c)** adjust per task
...
- `onp-spec plano <feature>` (if detection gets it wrong, force with
  `--agents opencode`) groups tasks on **disjoint files** into **parallel
  lanes** — 1 lane = 1 git worktree + 1 branch + 1 clean context window; with
  ...
  - `executar-tarefas.sh` — headless executor: runs `opencode run` with
    `--model` (`provider/model`) and `--variant` already set per task, `--format
    json` output (the stream goes to the ledger) and `--auto` (it must WRITE;
    `.opencode/opencode.json` permission rules still win), per lane in
    parallel or one task at a time in sequential mode, merges whatever needs
...
- **Table + summary every 1 minute (required while it runs)**: with the script
  ...
  script's terminal (`📣 resumo`). Also mirror the tasks in the todo list. The
  user is never left without knowing what's going on.
...
- Mirror the tasks in the todo list and keep updating the status — the user
  follows the progress in real time. In the mechanical tasks.md, use
...
- **"Can it run cheaper?"** → `onp-spec plano <feature> --modelo
  openai/gpt-5.6-luna --esforco baixo` (everything) or `onp-spec tarefa
  <feature> <T-xxx> --modelo <m> --esforco <level>` (per task, and regenerate
  the plan) — models and efforts ALWAYS pass through the user's confirmation
  before executing.
