...
export function ResultViewer({ result, files, rawResult, activeTab, onTab }: Props) {
  ...
  const download = (filename: string, value: string, type: string) => { const blob = new Blob([value], { type }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = filename; link.click(); URL.revokeObjectURL(url) }
  const markdown = activeTab === "summary" ? Object.entries(files).map(([name, content]) => `# ${name}\n\n${content}`).join("\n\n") : files[activeTab] ?? ""
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2"><button type="button" onClick={() => download("ade-report.json", JSON.stringify(rawResult, null, 2), "application/json")} className="rounded-lg bg-emerald-400 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-emerald-300">Download JSON</button><button type="button" onClick={() => download("ade-report.md", markdown, "text/markdown")} className="rounded-lg bg-emerald-400 px-4 py-2 text-sm font-semibold text-zinc-950 hover:bg-emerald-300">Download Markdown</button><button type="button" onClick={() => copy("JSON", JSON.stringify(rawResult, null, 2))} className="rounded-lg border border-zinc-700 px-4 py-2 text-sm text-zinc-200 hover:border-zinc-500">{copied === "JSON" ? "Copied JSON" : "Copy JSON"}</button><button type="button" onClick={() => copy("Markdown", markdown)} className="rounded-lg border border-zinc-700 px-4 py-2 text-sm text-zinc-200 hover:border-zinc-500">{copied === "Markdown" ? "Copied Markdown" : "Copy Markdown"}</button></div>
